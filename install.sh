...
{
    old_vimrc=$HOME"/.cofig/.vim/.vimrc"
...
{
    old_vimrc_plugins=$HOME"/.cofig/.vim/.vimrc.custom.plugins"
...
{
    old_vimrc_config=$HOME"/.cofig/.vim/.vimrc.custom.config"
...
{
    sudo dnf install -y vim ctags automake gcc gcc-c++ kernel-devel cmake python-devel python3-devel fontconfig ack git node npm
}

# 安装archlinux必备软件
function install_prepare_software_on_archlinux()
{
    sudo pacman -S --noconfirm vim ctags automake gcc cmake python3 python2 ack git fontconfig npm clang go
